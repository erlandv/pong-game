...
function getLabelEl(btn) {
  return btn.querySelector('.btn-label') || btn;
}

export function updateControls() {
  const startLabelEl = dom.startBtn.querySelector('.btn-label') || dom.startBtn;
  const pauseLabelEl = dom.pauseBtn.querySelector('.btn-label') || dom.pauseBtn;
  ...
  // start & running
  dom.startBtn.disabled = isRunningActive || runtime.gameOver;
  startLabelEl.textContent = isRunningActive ? 'Running...' : 'Start';
  dom.startBtn.dataset.icon = isRunningActive ? 'running' : 'start';

  // pause & resume
  const isResume = (runtime.paused && runtime.resumeCountdown === 0);
  pauseLabelEl.textContent = isResume ? 'Resume' : 'Pause';
  dom.pauseBtn.dataset.icon = isResume ? 'resume' : 'pause';
...
document.addEventListener('keydown', (e) => {
  ...
  if (e.code === 'Enter') {
    ...
    if (!runtime.gameRunning) {
      ...
      if (dom.hint) dom.hint.style.display = 0;
      hideResultOverlay();
      updateControls();
